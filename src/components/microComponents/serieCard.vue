<template>
    <div class="movie-card" @click="isHidden = !isHidden" v-click-outside="onClickOutside">
        ...
        <div class="movie-card_img">
            <img :src="card.poster_path ? `https://image.tmdb.org/t/p/w342/${card.poster_path}` : require(`../../assets/img/spallucce.png`)" :alt="`${card.original_name}.img`">
        </div>
        <div class="movie-card_text">
            <div class="movie-info_box">
                <h3 class="title">{{card.name}}</h3>
                <div class="original-title">(<img class="flag-img" :src="checkFlag(card.original_language) ? require(`../../assets/img/flags/${card.original_language}.png`) : require(`../../assets/img/flags/placeholder-flag-48.png`)" alt="flag.png">{{card.original_language.toUpperCase()}}: {{card.original_name}} )</div>
